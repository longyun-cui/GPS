...
use App\Repositories\ZY\ZYStaffRepository;

use Response, Auth, Validator, DB, Exception;
use QrCode, Excel;

class ZYStaffController extends Controller
{
    //
    private $service;
    private $repo;
    ...
    {
        $this->repo = new ZYStaffRepository;
    }
    ...
    public function login()
    {
        ...
        {
            $view_blade = env('TEMPLATE_ZY_STAFF').'entrance.login';
...
//            $admin = OrgAdministrator::whereEmail($email)->first();
            ...
            $user = ZY_User::withTrashed()->whereMobile($mobile)->first();
                ...
                if($user->deleted_at == null)
                {
                    if($user->active == 1)
                    {
                        ...
                        if(password_check($password,$user->password))
                        {
                            ...
                            if($remember) Auth::guard('zy_staff')->login($user,true);
                            else Auth::guard('zy_staff')->login($user,true);
