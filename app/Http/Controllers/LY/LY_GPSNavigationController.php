...
use App\Repositories\LY\LY_GPSNavigationRepository;

use Response, Auth, Validator, DB, Exception;
use QrCode, Excel;


class LY_GPSNavigationController extends Controller
{
    //
    private $service;
    private $repo;
    ...
    {
        $this->repo = new LY_GPSNavigationRepository();
    }
...
//            $admin = SuperAdministrator::whereEmail($email)->first();
    ...
    {
        return $this->repo->view_nav_index();
    }

    // 返回主页视图
    public function view_nav_404()
    {
        return $this->repo->view_nav_404();
    }
    ...
    {
//        dd(1);
        return $this->repo->view_nav_style();
    }
    ...
    {
        return $this->repo->view_nav_template_list();
    }

    // 测试
    public function view_nav_testing_list()
    {
        return $this->repo->view_nav_testing_list();
    }

    // 工具
    public function view_nav_tool_list()
    {
        return $this->repo->view_nav_tool_list();
    }
    ...
    {
        if(request()->isMethod('get')) return $this->repo->view_nav_todo_menu_create();
    ...
    {
        if(request()->isMethod('get')) return $this->repo->view_nav_todo_menu_edit();
        else if (request()->isMethod('post')) return $this->repo->operate_admin_todo_menu_save(request()->all());
    }
    // 【待办事-目录】返回-列表-视图（全部任务）
    public function view_nav_todo_menu_list()
    {
        if(request()->isMethod('get')) return $this->repo->view_nav_todo_menu_list(request()->all());
        else if(request()->isMethod('post')) return $this->repo->get_admin_todo_menu_list_datatable(request()->all());
    }
    // 【待办事-目录-修改记录】返回-列表-视图（全部任务）
    public function view_nav_todo_menu_modify_record()
    {
        if(request()->isMethod('get')) return $this->repo->view_nav_todo_menu_modify_record(request()->all());
    ...
    {
        if(request()->isMethod('get')) return $this->repo->view_nav_todo_item_create();
    ...
    {
        if(request()->isMethod('get')) return $this->repo->view_nav_todo_item_edit();
        else if (request()->isMethod('post')) return $this->repo->operate_admin_todo_item_save(request()->all());
    }
    // 【待办事-内容】返回-列表-视图（全部任务）
    public function view_nav_todo_item_list()
    {
        if(request()->isMethod('get')) return $this->repo->view_nav_todo_item_list(request()->all());
        else if(request()->isMethod('post')) return $this->repo->get_admin_todo_item_list_datatable(request()->all());
    }
    // 【待办事-内容-修改记录】返回-列表-视图（全部任务）
    public function view_nav_todo_item_modify_record()
    {
        if(request()->isMethod('get')) return $this->repo->view_nav_todo_item_modify_record(request()->all());
