...
use App\Models\GH\GH_Admin;

use App\Repositories\GH\WEB\GHWebAdminRepository;

use Response, Auth, Validator, DB, Exception;
use QrCode, Excel;

class GHWebAdminController extends Controller
{
    //
    private $repo;
    public function __construct()
    {
        $this->repo = new GHWebAdminRepository;
    }
    ...
    public function check_is_only_me()
    {
        ...
        if(Auth::guard('gh_web_admin')->check())
        {
            ...
            if(Auth::guard('gh_web_admin')->user()->admin_token == $token)
            {
    ...
    public function login()
    {
        ...
        {
            $view_blade = env('TEMPLATE_GH_WEB_ADMIN').'entrance.login';
...
//            }
     ...
     */
    // 【订单管理】返回-列表-视图（全部任务）
    public function view_item_product_list_for_all()
    {
        if(request()->isMethod('get')) return $this->repo->view_item_product_list_for_all(request()->all());
        else if(request()->isMethod('post')) return $this->repo->get_item_product_list_for_all_datatable(request()->all());
    }
    ...
    // 【订单管理】SELECT2 Client 客户
    public function operate_product_select2_client()
    {
        return $this->repo->operate_product_select2_client(request()->all());
    }
    // 【订单管理】SELECT2 Circle 环线
    public function operate_product_select2_circle()
    {
        return $this->repo->operate_product_select2_circle(request()->all());
    }
    // 【订单管理】SELECT2 Route 线路
    public function operate_product_select2_route()
    {
        return $this->repo->operate_product_select2_route(request()->all());
    }
    // 【订单管理】SELECT2 Pricing 定价
    public function operate_product_select2_pricing()
    {
        return $this->repo->operate_product_select2_pricing(request()->all());
    }
    // 【订单管理】SELECT2 Car 车辆
    public function operate_product_select2_car()
    {
        return $this->repo->operate_product_select2_car(request()->all());
    }
    // 【订单管理】SELECT2 Trailer 车挂
    public function operate_product_select2_trailer()
    {
        return $this->repo->operate_product_select2_trailer(request()->all());
    }
    // 【订单管理】SELECT2 Car 车辆
    public function operate_product_list_select2_car()
    {
        return $this->repo->operate_product_list_select2_car(request()->all());
    }
    // 【订单管理】SELECT2 Trailer 车挂
    public function operate_product_select2_driver()
    {
        return $this->repo->operate_product_select2_driver(request()->all());
    }


    // 【订单管理】添加
    public function operate_item_product_create()
    {
        if(request()->isMethod('get')) return $this->repo->view_item_product_create();
        else if (request()->isMethod('post')) return $this->repo->operate_item_product_save(request()->all());
    }
    // 【订单管理】编辑
    public function operate_item_product_edit()
    {
        if(request()->isMethod('get')) return $this->repo->view_item_product_edit();
        else if (request()->isMethod('post')) return $this->repo->operate_item_product_save(request()->all());
    }

    // 【订单管理】导入
    public function operate_item_product_import()
    {
        if(request()->isMethod('get')) return $this->repo->view_item_product_import();
        else if (request()->isMethod('post')) return $this->repo->operate_item_product_import_save(request()->all());
    }


    // 【订单管理】获取-详情
    public function operate_item_product_get()
    {
        return $this->repo->operate_item_product_get(request()->all());
    }
    // 【订单管理】获取-详情
    public function operate_item_product_get_html()
    {
        return $this->repo->operate_item_product_get_html(request()->all());
    }
    // 【订单管理】获取-附件
    public function operate_item_product_get_attachment_html()
    {
        return $this->repo->operate_item_product_get_attachment_html(request()->all());
    }


    // 【订单管理】删除
    public function operate_item_product_delete()
    {
        return $this->repo->operate_item_product_delete(request()->all());
    }
    // 【订单管理】发布
    public function operate_item_product_publish()
    {
        return $this->repo->operate_item_product_publish(request()->all());
    }
    // 【订单管理】完成
    public function operate_item_product_complete()
    {
        return $this->repo->operate_item_product_complete(request()->all());
    }
    // 【订单管理】弃用
    public function operate_item_product_abandon()
    {
        return $this->repo->operate_item_product_abandon(request()->all());
    }
    // 【订单管理】复用
    public function operate_item_product_reuse()
    {
        return $this->repo->operate_item_product_reuse(request()->all());
    }
    // 【内容】验证
    public function operate_item_product_verify()
    {
        return $this->repo->operate_item_product_verify(request()->all());
    }
    // 【内容】审核
    public function operate_item_product_inspect()
    {
        return $this->repo->operate_item_product_inspect(request()->all());
    }



    // 【订单管理】修改-文本-信息
    public function operate_item_product_info_text_set()
    {
        return $this->repo->operate_item_product_info_text_set(request()->all());
    }
    // 【订单管理】修改-时间-信息
    public function operate_item_product_info_time_set()
    {
        return $this->repo->operate_item_product_info_time_set(request()->all());
    }
    // 【订单管理】修改-option-信息
    public function operate_item_product_info_option_set()
    {
        return $this->repo->operate_item_product_info_option_set(request()->all());
    }
    // 【订单管理】修改-radio-信息
    public function operate_item_product_info_radio_set()
    {
        return $this->repo->operate_item_product_info_option_set(request()->all());
    }
    // 【订单管理】修改-select-信息
    public function operate_item_product_info_select_set()
    {
        return $this->repo->operate_item_product_info_option_set(request()->all());
    }
    // 【订单管理】添加-attachment-信息
    public function operate_item_product_info_attachment_set()
    {
        return $this->repo->operate_item_product_info_attachment_set(request()->all());
    }
    // 【订单管理】删除-attachment-信息
    public function operate_item_product_info_attachment_delete()
    {
        return $this->repo->operate_item_product_info_attachment_delete(request()->all());
    }
    // 【订单管理】修改-客户信息
    public function operate_item_product_info_client_set()
    {
        return $this->repo->operate_item_product_info_option_set(request()->all());
    }
    // 【订单管理】修改-车辆信息
    public function operate_item_product_info_car_set()
    {
        return $this->repo->operate_item_product_info_option_set(request()->all());
    }

    // 【订单管理】添加-行程记录
    public function operate_item_product_travel_set()
    {
        return $this->repo->operate_item_product_travel_set(request()->all());
    }
    ...
    public function view_item_product_finance_record()
    {
        if(request()->isMethod('get')) return $this->repo->view_item_product_finance_record(request()->all());
        else if(request()->isMethod('post')) return $this->repo->get_item_product_finance_record_datatable(request()->all());
    }

    // 【订单管理】添加-财务记录
    public function operate_item_product_finance_record_create()
    {
        return $this->repo->operate_item_product_finance_record_create(request()->all());
    }
    // 【订单管理】修改-财务记录
    public function operate_item_product_finance_record_edit()
    {
        return $this->repo->operate_item_product_finance_record_edit(request()->all());
    }
    ...
    public function view_item_product_modify_record()
    {
        if(request()->isMethod('get')) return $this->repo->view_item_product_modify_record(request()->all());
        else if(request()->isMethod('post')) return $this->repo->get_item_product_modify_record_datatable(request()->all());
    }
    ...
    // 【环线管理】SELECT2 product 订单
    public function operate_circle_select2_product_list()
    {
        return $this->repo->operate_circle_select2_product_list(request()->all());
    }
    // 【环线管理】SELECT2 Car 车辆
    ...
    // 【统计】返回-订单-数据
    public function get_statistic_data_for_product()
    {
        return $this->repo->get_statistic_data_for_product(request()->all());
    }
    // 【统计】返回-财务-数据
    ...
    // 【统计】订单-导出
    public function operate_statistic_export_for_product()
    {
        $this->repo->operate_statistic_export_for_product(request()->all());
    }
    // 【统计】订单-导出
    public function operate_statistic_export_for_product_by_ids()
    {
        $this->repo->operate_statistic_export_for_product_by_ids(request()->all());
    }
    // 【统计】环线-导出
