...
class ZYStaffRepository {
    ...
    public function __construct()
    {
        ...
        $this->view_blade_404 = env('TEMPLATE_ZY_STAFF').'errors.404';
    ...
    {
        if(Auth::guard("zy_staff")->check())
        {
            $this->auth_check = 1;
            $this->me = Auth::guard("zy_staff")->user();
...
//        $item_query = ZY_Item::with(['owner','creator','updater','completer']);
        $item_query = ZY_Task::with(['owner','creator','completer']);
...
//        dd($item_list->toArray());
        ...
        $view_blade = env('TEMPLATE_ZY_STAFF').'entrance.index';
...
//        $return['item_type'] = $item_type;
        ...
        else
        {
            ...
            if($mine)
            {
//                if(!in_array($mine->user_type,[11,88])) return view(env('TEMPLATE_ZY_STAFF').'errors.404');
...
//            ->where('active',1)
        ...
        $return['title_text'] = $title_text;
        return view(env('TEMPLATE_ZY_STAFF').'entrance.user.staff-list')->with($return);
    }
    // 【用户】【全部】返回-列表-数据
...
//            exit($e->getMessage());
    ...
    {
        $query = ZY_Item::select(['id','name as text'])->where(['item_category'=>100,'item_type'=>11]);
...
//        $return['item_type'] = $item_type;
    ...
    public function view_item_task_edit()
    {
        ...
        $mine = ZY_Item::with([])->withTrashed()->find($item_id);
...
//            else if($type == 'conception') $post_data["item_type"] = 91;
        ...
        {
            $mine = ZY_Item::withTrashed()->find($operate_id);
            if(!$mine) return response_error([],"该内容不存在，刷新页面重试！");
...
//            exit($e->getMessage());
    ...
    public function get_the_user_html($item)
    {
        ...
        // method A
        $item_html = view(env('TEMPLATE_ZY_STAFF').'component.user-list')->with($return)->__toString();
//        // method B
