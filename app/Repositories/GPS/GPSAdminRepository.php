...
//            exit($e->getMessage());
    ...
    public function view_admin_todo_menu_create()
    {
        ...
        $view_blade = env('TEMPLATE_GPS_ADMIN').'entrance.item.todo-menu-edit';
        return view($view_blade)->with($return);
    }
    // 【待办事-项目】返回-编辑-视图
    public function view_admin_todo_menu_edit()
    {
        ...
        else
        {
            ...
            if($mine)
            {
//                if($mine->deleted_at) return view(env('TEMPLATE_GPS_ADMIN').'entrance.errors.404');
//                else
                {
            ...
            else return view(env('TEMPLATE_GPS_ADMIN').'entrance.errors.404');
        }
    }
    // 【待办事-项目】保存数据
...
//            exit($e->getMessage());
    ...
    public function view_admin_todo_menu_list($post_data)
    {
        ...
        $view_blade = env('TEMPLATE_GPS_ADMIN').'entrance.item.todo-menu-list';
...
//        }
    ...
    public function view_admin_todo_item_create()
    {
        ...
        $view_blade = env('TEMPLATE_GPS_ADMIN').'entrance.item.todo-item-edit';
        return view($view_blade)->with($return);
    }
    // 【待办事-项目】返回-编辑-视图
...
//        $menu_list = $query->orderBy('updated_at','desc')->get();
        ...
        $view_blade = env('TEMPLATE_GPS_ADMIN').'entrance.item.todo-item-list';
        return view($view_blade)->with($view_data);
    }
    // 【待办事-项目】返回-列表-数据
...
//        if($item->owner_id != $me->id) return response_error([],"该内容不是你的，你不能操作！");
        ...
        if($data_type == "html")
        {
            $view_blade = env('TEMPLATE_GPS_ADMIN').'entrance.item.todo-info-html';
