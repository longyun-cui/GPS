...
class GPSDevUIRepository {
    ...
    public function __construct()
    {
        ...
        $this->view_blade_403 = env('TEMPLATE_GPS_DEV_UI').'entrance.errors.403';
        $this->view_blade_404 = env('TEMPLATE_GPS_DEV_UI').'entrance.errors.404';
...
//        dd($me->toArray());
        ...
        $view_blade = env('TEMPLATE_GPS_DEV_UI').'entrance.index';
    ...
    {
        $this->get_me();
        $view_blade = env('TEMPLATE_GPS_DEV_UI').'entrance.errors.404';
        return view($view_blade);
    }
...
//        dd($me->toArray());
        ...
        $view_blade = env('TEMPLATE_GPS_DEV_UI').'entrance.style';
        return view($view_blade);
    }
    ...
    public function view_my_profile_info_index()
    {
        ...
        $view_blade = env('TEMPLATE_GPS_DEV_UI').'entrance.my-account.my-profile-info-index';
    ...
    public function view_my_profile_info_edit()
    {
        ...
        $view_blade = env('TEMPLATE_GPS_DEV_UI').'entrance.my-account.my-profile-info-edit';
        return view($view_blade)->with($return);
    }
    // 【基本信息】保存数据
...
//            exit($e->getMessage());
    ...
    public function view_my_account_password_change()
    {
        ...
        $view_blade = env('TEMPLATE_GPS_DEV_UI').'entrance.my-account.my-account-password-change';
        return view($view_blade)->with($return);
    }
    // 【密码】保存数据
    ...
    public function view_admin_todo_menu_create()
    {
        ...
        $view_blade = env('TEMPLATE_GPS_DEV_UI').'entrance.item.todo-menu-edit';
        return view($view_blade)->with($return);
    }
    // 【待办事-项目】返回-编辑-视图
    public function view_admin_todo_menu_edit()
    {
        ...
        else
        {
            ...
            if($mine)
            {
//                if($mine->deleted_at) return view(env('TEMPLATE_GPS_DEV_UI').'entrance.errors.404');
//                else
                {
            ...
            else return view(env('TEMPLATE_GPS_DEV_UI').'entrance.errors.404');
        }
    }
    // 【待办事-项目】保存数据
...
//            exit($e->getMessage());
    ...
    public function view_admin_todo_menu_list($post_data)
    {
        ...
        $view_blade = env('TEMPLATE_GPS_DEV_UI').'entrance.item.todo-menu-list';
...
//        }
    ...
    public function view_admin_todo_item_create()
    {
        ...
        $view_blade = env('TEMPLATE_GPS_DEV_UI').'entrance.item.todo-item-edit';
        return view($view_blade)->with($return);
    }
    // 【待办事-项目】返回-编辑-视图
...
//        $menu_list = $query->orderBy('updated_at','desc')->get();
        ...
        $view_blade = env('TEMPLATE_GPS_DEV_UI').'entrance.item.todo-item-list';
        return view($view_blade)->with($view_data);
    }
    // 【待办事-项目】返回-列表-数据
...
//        if($item->owner_id != $me->id) return response_error([],"该内容不是你的，你不能操作！");
        ...
        if($data_type == "html")
        {
            $view_blade = env('TEMPLATE_GPS_DEV_UI').'entrance.item.todo-info-html';
