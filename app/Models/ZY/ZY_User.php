...
class ZY_User extends Authenticatable
{
    use Notifiable;
    use SoftDeletes;
...
//    protected $connection = 'mysql_def';
    ...
    function fans_list()
    {
        return $this->hasMany('App\Models\ZY\ZY_Pivot_User_Relation','relation_user_id','id');
    }
    ...
    function items()
    {
        return $this->hasMany('App\Models\ZY\ZY_Item','owner_id','id');
    }
    // 内容
    function ad_list()
    {
    ...
    function ad()
    {
        return $this->hasOne('App\Models\ZY\ZY_Item','id','advertising_id');
    }

    // 介绍
    function introduction()
    {
        return $this->hasOne('App\Models\ZY\ZY_Item','id','introduction_id');
    }

    // 与我相关的内容
    function pivot_item()
    {
        return $this->belongsToMany('App\Models\ZY\ZY_Item','pivot_user_item','user_id','item_id')
    ...
    function pivot_user()
    {
        return $this->belongsToMany('App\Models\ZY\ZY_User','pivot_user_user','user_1_id','user_2_id')
            ->withPivot(['active','relation_active','type','relation_type'])->withTimestamps();
    }
    ...
    {
        return $this->belongsToMany('App\Models\ZY\ZY_User','pivot_user_relation','mine_user_id','relation_user_id')
            ->withPivot(['active','relation_active','type','relation_type'])->withTimestamps();
    }
    ...
    {
        return $this->belongsToMany('App\Models\ZY\ZY_User','pivot_user_relation','relation_user_id','mine_user_id')
            ->withPivot(['active','relation_active','type','relation_type'])->withTimestamps();
    }
