<script>
    $(function() {
        ...
        // 【搜索】
        $("#datatable-for-product-list").on('click', ".filter-submit", function() {
            $('#datatable_ajax').DataTable().ajax.reload();
        });
        // 【刷新】
        $("#datatable-for-product-list").on('click', ".filter-refresh", function() {
            $('#datatable_ajax').DataTable().ajax.reload(null,false);
        });
        // 【重置】
        $("#datatable-for-product-list").on('click', ".filter-cancel", function() {
            $("#datatable-for-product-list").find('textarea.form-filter, input.form-filter, select.form-filter').each(function () {
                $(this).val("");
            });
...
//            $('select.form-filter').selectpicker('refresh');
            $("#datatable-for-product-list").find('select.form-filter option').attr("selected",false);
            $("#datatable-for-product-list").find('select.form-filter').find('option:eq(0)').attr('selected', true);

            $('#datatable_ajax').DataTable().ajax.reload();
        });
        // 【清空重选】
        $("#datatable-for-product-list").on('click', ".filter-empty", function() {
            $("#datatable-for-product-list").find('textarea.form-filter, input.form-filter, select.form-filter').each(function () {
                $(this).val("");
            });
            $(".select2-box").val(-1).trigger("change");
...
//            $('select.form-filter').selectpicker('refresh');
        ...
        // 【查询】回车
        $("#datatable-for-product-list").on('keyup', ".filter-keyup", function(event) {
            if(event.keyCode ==13)
            {
                $("#datatable-for-product-list").find(".filter-submit").click();
            }
        });


        // 【完整显示】
        $(".main-content").on('click', "#product-show-for-full", function() {
        ...
        // 【简要显示】
        $(".main-content").on('click', "#product-show-for-brief", function() {
        ...
        // 【财务显示】
        $(".main-content").on('click', "#product-show-for-finance", function() {
        ...
        // 【综合概览】【前一天】
        $(".main-content").on('click', ".date-pick-pre-for-product", function() {

            var $assign_dom = $('input[name="product-assign"]');
        ...
        // 【综合概览】【后一添】
        $(".main-content").on('click', ".date-pick-next-for-product", function() {
        ...
        $(".main-content").on('click', ".item-create-show", function() {
            var $that = $(this);
            $('#modal-body-for-product-create').modal('show');
        });

        // 【编辑】
        $(".main-content").on('click', ".item-create-link", function() {
            var $that = $(this);
            var $url = "/admin/item/product-create?&referrer="+encodeURIComponent(window.location.href);
        ...
        $(".main-content").on('click', ".item-edit-link", function() {
            var $that = $(this);
            var $url = "/admin/item/product-edit?id="+$that.attr('data-id')+"&referrer="+encodeURIComponent(window.location.href);
            window.location.href = $url;
            // window.open($url);
        });
...
//            $('input[name=id]').val($that.attr('data-id'));
            $('input[name=info-set-product-id]').val($that.attr('data-id'));
        ...
        $(".main-content").on('click', ".item-modal-show-for-detail-inspected", function() {
            ...
            $('input[name="detail-inspected-product-id"]').val($that.attr('data-id'));
            $('.info-detail-title').html($that.attr('data-id'));
            $('.info-set-title').html($that.attr('data-id'));
        ...
        $(".main-content").on('click', ".item-summit-for-detail-inspected", function() {
            ...
            var $id = $('input[name="detail-inspected-product-id"]').val();
            ...
            $.post(
                "{{ url('/item/product-inspect') }}",
                {
                    _token: $('meta[name="_token"]').attr('content'),
                    operate: "product-inspect",
                    item_id: $('input[name="detail-inspected-product-id"]').val(),
                ...
                function(data){
                    ...
                    else
                    {
                        ...
                        $row.find('td[data-key=product_status]').html('<small class="btn-xs bg-blue">已审核</small>');
        ...
        $(".main-content").on('click', ".item-delete-submit", function() {
            ...
            layer.msg('确定"删除"么？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/item/product-delete') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "product-delete",
        ...
        $(".main-content").on('click', ".item-abandon-submit", function() {
            ...
            layer.msg('确定"弃用"么？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/item/product-abandon') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "product-abandon",
                            item_id: $that.attr('data-id')
                        },
                        function(data){
                            layer.close(index);
        ...
        $(".main-content").on('click', ".item-reuse-submit", function() {
            ...
            layer.msg('确定"复用"么？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/item/product-reuse') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "product-reuse",
                            item_id: $that.attr('data-id')
                        },
                        function(data){
                            layer.close(index);
        ...
        $(".main-content").on('click', ".item-complete-submit", function() {
            ...
            layer.msg('确定"完成"么？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/item/product-complete') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "product-complete",
                            item_id: $that.attr('data-id')
                        },
                        function(data){
                            layer.close(index);
        ...
        $(".main-content").on('click', ".item-verify-submit", function() {
            ...
            layer.msg('确定"审核"么？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/item/product-verify') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "product-verify",
                            item_id: $that.attr('data-id')
                        },
                        function(data){
                            layer.close(index);
        ...
        $(".main-content").on('click', "#item-submit-for-travel-set", function() {
            ...
            layer.msg('确定"添加"么？', {
                ...
                ,yes: function(index){
                    $.post(
                        "{{ url('/item/product-travel-set') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: $('input[name="travel-set-operate"]').val(),
                            product_id: $('input[name="travel-set-product-id"]').val(),
        ...
        $(".main-content").on('dblclick', ".modal-show-for-info-text-set", function() {
            ...
            $('input[name=info-text-set-product-id]').val($that.attr("data-id"));
        ...
        $(".main-content").on('click', "#item-submit-for-info-text-set", function() {
            ...
            // });
                    ...
                    $.post(
                        "{{ url('/item/product-info-text-set') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: $('input[name="info-text-set-operate"]').val(),
                            product_id: $('input[name="info-text-set-product-id"]').val(),
        ...
        $(".main-content").on('dblclick', ".modal-show-for-info-time-set", function() {
            ...
            $('input[name=info-time-set-product-id]').val($that.attr("data-id"));
        ...
        $(".main-content").on('click', "#item-submit-for-info-time-set", function() {
            ...
            // });
                    ...
                    $.post(
                        "{{ url('/item/product-info-time-set') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: $('input[name="info-time-set-operate"]').val(),
                            product_id: $('input[name="info-time-set-product-id"]').val(),
        ...
        $(".main-content").on('dblclick', ".modal-show-for-info-radio-set", function() {
            ...
            $('input[name=info-radio-set-product-id]').val($that.attr("data-id"));
        ...
        $(".main-content").on('click', "#item-submit-for-info-radio-set", function() {
            ...
            // });
                    ...
                    $.post(
                        "{{ url('/item/product-info-radio-set') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: $('input[name="info-radio-set-operate"]').val(),
                            product_id: $('input[name="info-radio-set-product-id"]').val(),
...
//                                $('#datatable_ajax_inner').DataTable().ajax.reload(null, false);
        ...
        $(".main-content").on('dblclick', ".modal-show-for-info-select-set", function() {
            ...
            $('input[name=info-select-set-product-id]').val($that.attr("data-id"));
...
//            $('select[name=info-select-set-column-value]').find("option").eq(0).attr("selected","selected");
        ...
        $(".main-content").on('click', "#item-submit-for-info-select-set", function() {
            ...
            // });
                    ...
                    $.post(
                        "{{ url('/item/product-info-select-set') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: $('input[name="info-select-set-operate"]').val(),
                            product_id: $('input[name="info-select-set-product-id"]').val(),
...
//                                $('#datatable_ajax_inner').DataTable().ajax.reload(null, false);
        ...
        $(".main-content").on('dblclick', ".modal-show-for-attachment", function() {
            ...
            $('input[name=attachment-set-product-id]').val($that.attr("data-id"));
            ...
            $('#modal-attachment-set-form input[name=product_id]').val($that.attr("data-id"));
            ...
            $.ajax({
                ...
                dataType:'json',
                async:false,
                url: "{{ url('/item/product-get-attachment-html') }}",
                data: {
                    _token: $('meta[name="_token"]').attr('content'),
                    operate:"item-get",
                    product_id: $that.attr('data-id')
        ...
        $(".main-content").on('click', "#item-submit-for-attachment-set", function() {
            ...
            var $index = layer.load(1, {
                ...
                success: function (layer) {
                    ...
                    var options = {
                        url: "{{ url('/item/product-info-attachment-set') }}",
                        type: "post",
                        dataType: "json",
                        // target: "#div2",
                        success: function (data) {
...
//                                });
                                ...
                                $.ajax({
                                    ...
                                    data: {
                                        _token: $('meta[name="_token"]').attr('content'),
                                        operate:"item-get",
                                        product_id: $('#modal-attachment-set-form input[name=product_id]').val()
                                    },
                                    success:function(data){
                                        if(!data.success)
                                        {
...
//                                $('#datatable_ajax_inner').DataTable().ajax.reload(null, false);
        ...
        // 【附件-attachment-属性】【删除】
        $(".main-content").on('click', ".product-attachment-delete-this", function() {
            var $that = $(this);
            layer.msg('确定"删除"么？', {
                time: 0
                ,btn: ['确定', '取消']
                ,yes: function(index){
                    $.post(
                        "{{ url('/item/product-info-attachment-delete') }}",
                        {
                            _token: $('meta[name="_token"]').attr('content'),
                            operate: "product-attachment-delete",
                            item_id: $that.attr('data-id')
                        },
                        function(data){
                            // layer.close(index);
...
//                                $('#datatable_ajax').DataTable().ajax.reload(null,false);
        ...
        $(".main-content").on('click', '#bulk-submit-for-export', function() {
            ...
            var $url = url_build('/statistic/statistic-export-for-product-by-ids?ids='+$ids);
            window.open($url);
        ...
        // select2 项目
        $('.product-select2-project').select2({
            ajax: {
                url: "{{ url('/item/item_select2_project') }}",
                dataType: 'json',
                delay: 250,
